...
var handler = async function(event) {
  ...
  const deviceId = event.queryStringParameters?.deviceId || event.pathParameters?.deviceId || (event.body ? JSON.parse(event.body).deviceId : null);
  if (!deviceId) {
    return {
      statusCode: 400,
      body: JSON.stringify({
...
var generateTestCoordinates = (deviceId) => {
